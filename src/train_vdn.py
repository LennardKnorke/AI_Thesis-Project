...
from agents import *
from config import (
    HYPERSEARCH_RESULTS_DIR, RESULTS_DIR, 
    generate_param_grid, load_best_params,
    TRAINING_EPISODES_FINAL, TRAINING_EPISODES_HYPERSEARCH,
    NUM_RUNS
)
...
vdn_grid = generate_param_grid(vdn_grid)


def run_vdn_training(
        params : dict, 
        save_path : str, 
        episodes : int,
    ...
    if os.path.exists(path):
        tqdm.write(f"Results already exist! - {path}")
        return None, None

    final_agents : dict[str, VDN_CentralPlanner] = {}
    ...
    for g in pbar:
        ...
        for n in pbar2:
            ...
            _agents = VDN_CentralPlanner(env, g, env.num_cards, env.num_actions, **params)
    ...
    # Final Save
    if save_policy:
        for key, ag in final_agents.items():
            _path0 = os.path.join(save_path, f"{key}_agents.pkl")
            ag.save(_path0)
...
def hyper_search_vdn():
    best_params = None
    best_results = None
    best_param_path = os.path.join(_results_dir, "best_params.json")

    pbar = tqdm(enumerate(vdn_grid), desc="Hypersearch_vdn", leave=False)
    for i, param_set in pbar:
        ...
        results, _ = run_vdn_training(param_set, path, TRAINING_EPISODES_HYPERSEARCH, False)
        ...
        if is_better_result:
            ...
            tqdm.write(
                f"[VDN hypersearch] new best @ config {i}: "
                f"avg reward = {compute_avg_final_reward(results):.4f} | params = {param_set}"
            )
    return

def train_vdn():
    print("VDN Training Pipeline")
    hyper_search_vdn()

    best_params = load_best_params(os.path.join(_results_dir, "best_params.json"))
    run_vdn_training(best_params, _results_dir, TRAINING_EPISODES_FINAL, save_policy=True)
    return
